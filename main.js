...
(function () {
  ...
  function setupProductModalFromPages() {
    ...
    // ✅ 你指定的 TOC / 內容固定順序（缺少的項目自動略過，但順序不變）
    // 1 容量／規格
    // 2 成份
    // 3 適用對象／族群
    // 4 建議使用方式（使用方向）
    ...
    // 8 先聊聊狀況
    const SECTION_ORDER = [
      { key: 'spec', label: '容量／規格' },
      { key: 'ingredient', label: '成份' },
      { key: 'audience', label: '適用對象／族群' },
      { key: 'usage', label: '建議使用方式（使用方向）' },
      { key: 'storage', label: '保存方式' },
      { key: 'cooperate', label: '合作方式與報價' },
      { key: 'faq', label: '常見問題' },
      { key: 'consult', label: '先聊聊狀況' }
    ];
    ...
    function getSectionKeyByTitle(title) {
      const nt = normalizeTitle(title);
      // 兼容舊寫法與常見變體（保守比對，避免誤判）
      if (/容量|規格|重量|包裝|內容量/.test(nt)) return 'spec';
      if (/成份|成分|原料|配方|內容物/.test(nt)) return 'ingredient';
      if (/適用對象|適合族群|族群|對象/.test(nt)) return 'audience';
      if (/建議使用方式|使用方式|使用方向|吃法|用法/.test(nt)) return 'usage';
      if (/保存方式|保存|存放|冷藏|常溫/.test(nt)) return 'storage';
      if (/合作方式|報價|合作|經銷|批發|零售|詢價/.test(nt)) return 'cooperate';
      if (/常見問題|FAQ|Q&A/i.test(nt)) return 'faq';
      if (/先聊聊狀況|聊聊狀況|諮詢|再評估/.test(nt)) return 'consult';
      return null;
    }

    function sectionHasMeaningfulContent(sectionEl) {
      if (!sectionEl) return false;

      // 若除了標題外，還有任一內容元素，就視為有內容
      const clones = sectionEl.cloneNode(true);
      clones.querySelectorAll('h1,h2,h3').forEach((h) => h.remove());

      // 移除純粹的空白
      const text = (clones.textContent || '').replace(/\s+/g, '').trim();
      if (text.length > 0) return true;

      // 即使沒有文字，只要有圖片/按鈕/列表等，也算有內容
      if (clones.querySelector('img, ul, ol, a, button, table')) return true;
      return false;
    }

    // ✅ 將彈窗內容依 SECTION_ORDER 重新排序；空節直接移除
    function reorderModalSections(modalPageRoot) {
      if (!modalPageRoot) return;

      const sections = Array.from(modalPageRoot.querySelectorAll('section.section'));
      if (sections.length <= 1) return;

      // 第一個 section 多半是「產品介紹/主視覺/前言」，保留在最上方
      const intro = sections[0];
      const rest = sections.slice(1);

      const buckets = new Map();
      SECTION_ORDER.forEach((s) => buckets.set(s.key, []));
      const unknown = [];

      rest.forEach((sec) => {
        const h2 = sec.querySelector('h2');
        const key = getSectionKeyByTitle(h2 ? h2.textContent : '');
        ...
        if (key && buckets.has(key)) buckets.get(key).push(sec);
      ...
      // 清空並重組
      while (modalPageRoot.firstChild) modalPageRoot.removeChild(modalPageRoot.firstChild);
      modalPageRoot.appendChild(frag);
    }
    ...
    function buildToc(container) {
      // 只用「每個 section 內第一個 h2」作為跳轉點，避免出現空白 pill
      ...
      const firstH2ByKey = new Map();
      sections.forEach((sec) => {
        const h2 = sec.querySelector('h2');
        if (!h2) return;
        const key = sectionKeyFromHeadingText(h2.textContent || '');
        if (!key) return;
        if (!sectionHasMeaningfulContent(sec)) return;
        if (!firstH2ByKey.has(key)) firstH2ByKey.set(key, h2);
      });

      const frag = document.createDocumentFragment();
      SECTION_ORDER.forEach((it) => {
        const h2 = firstH2ByKey.get(it.key);
        if (!h2) return;

        // 確保有 id 才能跳轉
        if (!h2.id) h2.id = slugifyId(h2.textContent || it.label);
    ...
    async function loadPageIntoModal(href, fallbackTitle) {
      ...
      try {
        ...
        // ✅ 彈窗內容依 8 項順序重排，並移除「只有標題、沒有內文」的空段落
