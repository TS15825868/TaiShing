...
(function () {
  ...
  function setupProductModalFromPages() {
    ...
    /* =====================================================
       ✅ 強化：直接綁定 .js-product-modal
       - 防止某些頁面/瀏覽器環境下，事件委派被覆蓋層或 stopPropagation 影響，
         造成「點產品不跳窗」。
       - 只要 <a> 具備 .js-product-modal，就攔截並開啟彈窗。
       ===================================================== */
    function bindExplicitModalLinks(root=document){
      const links = root.querySelectorAll('a.js-product-modal[href]');
      links.forEach((a) => {
        if(a.__productModalBound) return;
        a.__productModalBound = true;
    ...
    // 先把頁面上已存在的觸發器都綁好
    bindExplicitModalLinks(document);
